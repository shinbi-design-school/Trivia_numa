...
public class Mysql extends Quiz {
	...
	private static String[][] questions = {
			{"データベースは何のために使いますか？","image-program/1.jpg"},
			{"テーブルはデータをどう表現しますか？","image-program/2.jpeg"},
			{"SQLは何のために使う言語ですか？","image-program/3.jpg"},
			{"主キーは何のために使いますか？","image-program/4.jpeg"},
			{"外部キーは何を示しますか？","image-program/5.jpg"},
			{"データを取得するためのSQL文はどれですか？","image-program/6.jpg"},
			{"新しいデータをテーブルに追加するためのSQL文はどれですか？","image-program/7.jpg"},
			{"データを削除するためのSQL文はどれですか？","image-program/8.jpg"},
			{"データベースの一連の操作をまとめて扱う単位を何と呼びますか？","image-program/9.jpg"},
			{"特定のクエリの結果を仮想的なテーブルとして保存することを何と呼びますか？","image-program/10.jpg"}
	    };
	 ...
	 private static String[] explanations = {
		        ...
		        "INSERT文は、テーブルに新しいデータを追加するためのSQL文です。例えば、顧客テーブルに新しい顧客情報を追加する場合、INSERT INTO customers (name, email) VALUES ('John Doe', 'john@example.com');と書くことで、顧客テーブルに新しい行が追加されます。",
		        "DELETE文は、テーブルからデータを削除するためのSQL文です。例えば、顧客テーブルから特定の顧客を削除する場合、DELETE FROM customers WHERE id = 1;と書くことで、顧客IDが1のレコードを削除します。条件（WHERE句）を指定しないと、テーブル内の全てのデータが削除されるため注意が必要です。",
		        ...
		        "ビューは、特定のクエリの結果を仮想的なテーブルとして保存する機能です。ビューは実際のデータを持たず、SQLクエリによって定義されます。例えば、特定のカラムだけを選択して表示するためにビューを使うことができます。ビューを使うことで、複雑なクエリを簡単に再利用したり、ユーザーに必要なデータのみを提供したりすることができます。"
		    };
